...
var Sortable = {
  ...
  create: function(table) {
    ...
    for (var i=0; i < firstRow.cells.length; i++) {
      ...
      if (cell.className.indexOf("unsort") != -1 || cell.className.indexOf("nosort") != -1 || Sortable.trim(cell.innerHTML) == '') continue;
      cell.className = "sort";
      cell.table=table;
      cell.key = 'key' + i;
      cell.onclick = function () {Sortable.sort(this.table, this.key , this.reverse); this.reverse = !this.reverse ; return false;};
  ...
  key: function(text) {
    ...
    // num, parseFloat() will accept sortable dates 1903-10-02 as a number 1903
    ...
    // text
      ...
      text=text.toLowerCase().replace(/[\[\]\P{L}]/g, '').replace(/œ/g, 'oe').replace(/æ/g, 'ae').replace(/ç/g, 'c').replace(/ñ/g, 'n').replace(/[éèêë]/g, 'e').replace(/[àâä]/g, 'a').replace(/[ïîí]/g, 'i').replace(/úûü/, 'u') +"____________________";
...
if(window.document.body) {
  ...
  window.addEventListener('load', Sortable.load, false);
}
else if (window.attachEvent) {
  window.attachEvent('onload', Sortable.load);
}
